<%@ taglib prefix="s" uri="http://www.springframework.org/tags"%><%@taglib
	...
	<div class="medium button split dropdown eight columns">
			...
			<c:forEach var="name" items="${categoryMap.keySet()}">
				<c:if test="${categoryMap.get(name) != catId }">
					...
					<security:authorize access="hasRole('A')">
						<c:if test="${categoryMap.get(name) != 0 }">
					    	<li style="background-color: #C60F13"><a style="font-weight: bold; color: white;" href="<s:url value="/admin/branch/${categoryMap.get(name)}/course" />">Create New Course</a></li>
					    </c:if>
					</security:authorize>					
					<li class="divider"></li>
				</c:if>
	...
	<div class="three columns offset-by-one right">
				...
				<div class="six columns">
					<div class="medium alert button split dropdown right">
					  <a href='<s:url value="/course/${course.id}" />'>Course Details</a>
					    <span></span>
					  <ul>
					    <li><a href="<s:url value="/course/${course.id}/register" />">Register</a></li>
						<security:authorize access="hasRole('A')">
						<li class="divider"></li>
